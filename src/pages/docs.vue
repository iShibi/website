...
const watchRoute = async () => {
	...
	if (docs.value?.id !== source.value?.id && docs.value?.tag !== tag.value) {
		// @ts-ignore
		await fetchDocs(sources[route.params.source] ?? MainSource, tag.value ?? MainSource.defaultTag);
	}
